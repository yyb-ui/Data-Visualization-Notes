...
*· 核心原理：adjustText 的运作模式是 “先收集，后统一计算”。你不能像以前那样直接画好标签就结束，而是要把所有标签的生成对象（t）存入一个列表 texts，等所有点都跑完，再一次性把列表扔给底层的物理引擎去计算“谁该往哪里避让”。这种批量处理是它能实现全局优化的根基。*
...
*· 为什么必须锁死方向？ 在散点图中，如果 X 轴代表的是训练耗时（Training Time） 这类绝对的物理指标，标签是绝不可以左右乱跑的，否则图表的时间尺度就完全错乱了。only_move={'text': 'y', 'points': 'y'} 强制规定：只能上下（Y 轴）弹开，绝对不动 X 轴。 这是画这张图最核心的工程底线。*

*第三步：调参实战——解决“字还粘在点上”的问题*

```python
force_text=3.0        # 排斥力（默认 1.0）
expand_text=(2.0, 2.0) # 文字边界气囊
force_points=1.5      # 允许点微微让步
```

*· force_text=3.0（排斥力弹簧）：默认值是 1.0，如果标签还是死死粘在点上，说明推力不够。把它提高到 3.0 或 5.0，相当于把弹簧拧紧，文字会瞬间向外“弹开”。*
*· expand_text=(2.0, 2.0)（碰撞气囊）：即使文字本身没重叠，但如果它们之间的“隐形保护圈”碰到了，也会触发避让。把这个气囊调大，能让密集的点更顺畅地散开。*
*· force_points=1.5（微小让步）：如果文字被推开太远，可以允许散点本身也产生极轻微的移动，双方互相让出空间，避让会更丝滑。*
...
*· 为什么需要牵引线？ 标签被推开后，读者可能找不到它原本属于哪个点了。加上这个参数，系统会自动为被推开的标签绘制一条半透明的灰色指引线，精准连回它的原点。这根细线能让你的散点图瞬间具备“顶级学术期刊论文配图”的高级质感。*
